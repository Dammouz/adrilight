    ...
    */
    ...
    public class NightLightStatusWatcher
    {
        ...
        internal void TrainAndGuess()
        {
            ...
            //var trainingPipeLine = mlContext.Transforms.Conversion.ConvertType("Data")
            //    .AppendCacheCheckpoint(mlContext)
            //    .Append(mlContext.BinaryClassification.Trainers.StochasticDualCoordinateAscent(
            //                labelColumnName: nameof(NightLightDataRow.IsActive),
        ...
        private void WatchOnce()
        {
            ...
            if (_lastData != null && _lastData.SequenceEqual(data))
            {
                return;
            }
            _lastData = data;
        ...
        {
            if (onByteArrays.Any(onByteArray => onByteArray.SequenceEqual(data)))
            {
                return NightLightStatusEnum.On;
            }

            if (offByteArrays.Any(offByteArray => offByteArray.SequenceEqual(data)))
            {
                return NightLightStatusEnum.Off;
            }
